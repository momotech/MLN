...
@class MLN_GCDAsyncReadPacket;
@class MLN_GCDAsyncWritePacket;
@class MLN_MLN_GCDAsyncSocketPreBuffer;
@protocol MLN_GCDAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const MLN_GCDAsyncSocketException;
extern NSString *const MLN_GCDAsyncSocketErrorDomain;

extern NSString *const MLN_GCDAsyncSocketQueueName;
extern NSString *const MLN_GCDAsyncSocketThreadName;

extern NSString *const MLN_GCDAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const MLN_GCDAsyncSocketUseCFStreamForTLS;
#endif
#define MLN_GCDAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define MLN_GCDAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define MLN_GCDAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const MLN_GCDAsyncSocketSSLPeerID;
extern NSString *const MLN_GCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const MLN_GCDAsyncSocketSSLProtocolVersionMax;
extern NSString *const MLN_GCDAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const MLN_GCDAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const MLN_GCDAsyncSocketSSLDiffieHellmanParameters;
#endif

#define MLN_GCDAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, MLN_GCDAsyncSocketError) {
	MLN_GCDAsyncSocketNoError = 0,           // Never used
	MLN_GCDAsyncSocketBadConfigError,        // Invalid configuration
	MLN_GCDAsyncSocketBadParamError,         // Invalid parameter was passed
	MLN_GCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	MLN_GCDAsyncSocketReadTimeoutError,      // A read operation timed out
	MLN_GCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	MLN_GCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	MLN_GCDAsyncSocketClosedError,           // The remote peer closed the connection
	MLN_GCDAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface MLN_GCDAsyncSocket : NSObject

/**
 * MLN_GCDAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, MLN_GCDAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<MLN_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<MLN_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create MLN_GCDAsyncSocket from already connect BSD socket file descriptor
**/
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<MLN_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError **)error;

#pragma mark Configuration

@property (atomic, weak, readwrite, nullable) id<MLN_GCDAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<MLN_GCDAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<MLN_GCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<MLN_GCDAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<MLN_GCDAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means MLN_GCDAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means MLN_GCDAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, MLN_GCDAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, MLN_GCDAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of MLN_GCDAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a MLN_GCDAsyncSocket instance (e.g. in a dealloc method)
...
@property (atomic, readonly) BOOL isAccepted;
 ...
 * it is treated similarly to a timeout - the socket is closed with a MLN_GCDAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that MLN_GCDAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - MLN_GCDAsyncSocketManuallyEvaluateTrust
 ...
 *     So MLN_GCDAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - MLN_GCDAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default MLN_GCDAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct MLN_GCDAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, MLN_GCDAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by MLN_GCDAsyncSocket,
 ...
 *
 * - MLN_GCDAsyncSocketSSLProtocolVersionMin
 * - MLN_GCDAsyncSocketSSLProtocolVersionMax
 ...
 * 
 * - MLN_GCDAsyncSocketSSLSessionOptionFalseStart
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionFalseStart.
 * 
 * - MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     See Apple's documentation for kSSLSessionOptionSendOneByteRecord.
 * 
 * - MLN_GCDAsyncSocketSSLCipherSuites
 ...
 *
 * - MLN_GCDAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see MLN_GCDAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use MLN_GCDAsyncSocketSSLProtocolVersionMin & MLN_GCDAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * MLN_GCDAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(MLN_GCDAsyncSocket *)sock
 * {
...
@protocol MLN_GCDAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(MLN_GCDAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(MLN_GCDAsyncSocket *)sock didConnectToUrl:(NSURL *)url;
...
**/
- (void)socket:(MLN_GCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(MLN_GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(MLN_GCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(MLN_GCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(MLN_GCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(MLN_GCDAsyncSocket *)sock;
 ...
 * 
 * Note: If the MLN_GCDAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socket:(MLN_GCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
