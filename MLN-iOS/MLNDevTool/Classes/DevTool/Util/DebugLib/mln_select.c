...
\*=========================================================================*/
static mln_t_socket getfd(lua_State *L);
static int dirty(lua_State *L);
static void collect_fd(lua_State *L, int tab, int itab, 
        fd_set *set, mln_t_socket *max_fd);
static int check_dirty(lua_State *L, int tab, int dtab, fd_set *set);
static void return_fd(lua_State *L, fd_set *set, mln_t_socket max_fd, 
...
* Initializes module
\*-------------------------------------------------------------------------*/
int mln_select_open(lua_State *L) {
...
static int global_select(lua_State *L) {
    ...
    mln_t_socket max_fd = SOCKET_INVALID;
    fd_set rset, wset;
    mln_t_timeout tm;
    ...
    t = ndirty > 0? 0.0: t;
    mln_timeout_init(&tm, t, -1);
    mln_timeout_markstart(&tm);
    ret = mln_socket_select(max_fd+1, &rset, &wset, NULL, &tm);
    if (ret > 0 || ndirty > 0) {
...
\*=========================================================================*/
static mln_t_socket getfd(lua_State *L) {
    mln_t_socket fd = SOCKET_INVALID;
    ...
    if (!lua_isnil(L, -1)) {
        ...
        if (lua_isnumber(L, -1)) {
            ...
            fd = (numfd >= 0.0)? (mln_t_socket) numfd: SOCKET_INVALID;
        }
    } 
    lua_pop(L, 1);
    return fd;
