...
static int inet_gethost(const char *address, struct hostent **hp) {
    ...
    if (inet_aton(address, &addr))
        return mln_socket_gethostbyaddr((char *) &addr, sizeof(addr), hp);
    else
        return mln_socket_gethostbyname(address, hp);
}
...
static int inet_global_tohostname(lua_State *L) {
    ...
    if (err != IO_DONE) {
        lua_pushnil(L);
        lua_pushstring(L, mln_socket_hoststrerror(err));
        return 2;
    }
...
static int inet_global_getnameinfo(lua_State *L) {
    ...
    if (ret != 0) {
        lua_pushnil(L);
        lua_pushstring(L, mln_socket_gaistrerror(ret));
        return 2;
    }
...
static int inet_global_gethostname(lua_State *L)
{
    ...
    if (gethostname(name, 256) < 0) {
        lua_pushnil(L);
        lua_pushstring(L, mln_socket_strerror(errno));
        return 2;
    } else {
...
int mln_inet_meth_getpeername(lua_State *L, mln_p_socket ps, int family)
{
    ...
    if (getpeername(*ps, (mln_SA *) &peer, &peer_len) < 0) {
        lua_pushnil(L);
        lua_pushstring(L, mln_socket_strerror(errno));
        return 2;
    }
...
int mln_inet_meth_getsockname(lua_State *L, mln_p_socket ps, int family)
{
    ...
    char name[INET6_ADDRSTRLEN];
    char port[6]; /* 65535 = 5 bytes + 0 to terminate it */
    if (getsockname(*ps, (mln_SA *) &peer, &peer_len) < 0) {
        lua_pushnil(L);
        lua_pushstring(L, mln_socket_strerror(errno));
        return 2;
    }
...
const char *mln_inet_trycreate(mln_p_socket ps, int family, int type) {
    return mln_socket_strerror(mln_socket_create(ps, family, type, 0));
}
...
const char *mln_inet_trydisconnect(mln_p_socket ps, int family, mln_p_timeout tm)
{
    switch (family) {
        case PF_INET: {
            ...
            return mln_socket_strerror(mln_socket_connect(ps, (mln_SA *) &sin, 
                sizeof(sin), tm));
        }
        case PF_INET6: {
            ...
            return mln_socket_strerror(mln_socket_connect(ps, (mln_SA *) &sin6, 
                sizeof(sin6), tm));
        }
    }
    return NULL;
        ...
        const char *serv, mln_p_timeout tm, struct addrinfo *connecthints)
{
    ...
    const char *err = NULL;
    /* try resolving */
    err = mln_socket_gaistrerror(getaddrinfo(address, serv,
    ...
    for (iterator = resolved; iterator; iterator = iterator->ai_next) {
        ...
        if (*family != iterator->ai_family) {
            mln_socket_destroy(ps);
            err = mln_socket_strerror(mln_socket_create(ps, iterator->ai_family, 
            ...
            *family = iterator->ai_family;
            ...
            mln_socket_setnonblocking(ps);
        }
        /* try connecting to remote address */
        err = mln_socket_strerror(mln_socket_connect(ps, (mln_SA *) iterator->ai_addr, 
...
\*-------------------------------------------------------------------------*/
const char *mln_inet_tryaccept(mln_p_socket server, int family, mln_p_socket client, 
    mln_p_timeout tm)
{
	socklen_t len;
	mln_t_sockaddr_storage addr;
	if (family == PF_INET6) {
		len = sizeof(struct sockaddr_in6);
	} else {
		len = sizeof(struct sockaddr_in);
	}
	return mln_socket_strerror(mln_socket_accept(server, client, (mln_SA *) &addr, 
        &len, tm));
}
        ...
        struct addrinfo *bindhints)
{
    struct addrinfo *iterator = NULL, *resolved = NULL;
    const char *err = NULL;
    mln_t_socket sock = *ps;
    /* translate luasocket special values to C */
    if (strcmp(address, "*") == 0) address = NULL;
    if (!serv) serv = "0";
    /* try resolving */
    err = mln_socket_gaistrerror(getaddrinfo(address, serv, bindhints, &resolved));
    if (err) {
        if (resolved) freeaddrinfo(resolved);
        return err;
    }
    ...
    for (iterator = resolved; iterator; iterator = iterator->ai_next) {
        if(sock == SOCKET_INVALID) {
            err = mln_socket_strerror(mln_socket_create(&sock, iterator->ai_family,
                        iterator->ai_socktype, iterator->ai_protocol));
            if(err)
                continue;
        }
        /* try binding to local address */
        err = mln_socket_strerror(mln_socket_bind(&sock,
            (mln_SA *) iterator->ai_addr,
            (socklen_t) iterator->ai_addrlen));

        /* keep trying unless bind succeeded */
        if (err) {
            if(sock != *ps)
                mln_socket_destroy(&sock);
        } else {
