...
int mln_socket_waitfd(mln_p_socket ps, int sw, mln_p_timeout tm) {
    ...
    do {
        int t = (int)(mln_timeout_getretry(tm)*1e3);
...
int mln_socket_select(mln_t_socket n, fd_set *rfds, fd_set *wfds, fd_set *efds, 
    ...
    do {
        struct timeval tv;
        double t = mln_timeout_getretry(tm);
        tv.tv_sec = (int) t;
        tv.tv_usec = (int) ((t - tv.tv_sec) * 1.0e6);
...
\*-------------------------------------------------------------------------*/
int mln_socket_bind(mln_p_socket ps, mln_SA *addr, socklen_t len) {
    int err = IO_DONE;
    mln_socket_setblocking(ps);
...
\*-------------------------------------------------------------------------*/
int mln_socket_connect(mln_p_socket ps, mln_SA *addr, socklen_t len, mln_p_timeout tm) {
    int err;
    ...
    err = mln_socket_waitfd(ps, WAITFD_C, tm);
    if (err == IO_CLOSED) {
...
* Accept with timeout
\*-------------------------------------------------------------------------*/
int mln_socket_accept(mln_p_socket ps, mln_p_socket pa, mln_SA *addr, socklen_t *len, mln_p_timeout tm) {
    if (*ps == SOCKET_INVALID) return IO_CLOSED; 
    for ( ;; ) {
        int err;
        ...
        if ((err = mln_socket_waitfd(ps, WAITFD_R, tm)) != IO_DONE) return err;
    }
    /* can't reach here */
    return IO_UNKNOWN;
}
        ...
        size_t *sent, mln_p_timeout tm)
{
    ...
    *sent = 0;
    ...
    for ( ;; ) {
        ...
        if ((err = mln_socket_waitfd(ps, WAITFD_W, tm)) != IO_DONE) return err;
    }
    /* can't reach here */
    return IO_UNKNOWN;
}
...
* Sendto with timeout
\*-------------------------------------------------------------------------*/
int mln_socket_sendto(mln_p_socket ps, const char *data, size_t count, size_t *sent, 
        mln_SA *addr, socklen_t len, mln_p_timeout tm)
{
    int err;
    *sent = 0;
    if (*ps == SOCKET_INVALID) return IO_CLOSED;
...
\*-------------------------------------------------------------------------*/
int mln_socket_recvfrom(mln_p_socket ps, char *data, size_t count, size_t *got, 
        mln_SA *addr, socklen_t *len, mln_p_timeout tm) {
    int err;
    *got = 0;
    if (*ps == SOCKET_INVALID) return IO_CLOSED;
...
const char *mln_socket_hoststrerror(int err) {
    if (err <= 0) return mln_io_strerror(err);
    switch (err) {
...
const char *mln_socket_ioerror(mln_p_socket ps, int err) {
    (void) ps;
    return mln_socket_strerror(err);
} 
