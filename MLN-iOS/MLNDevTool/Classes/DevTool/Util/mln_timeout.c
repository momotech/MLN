...
double mln_timeout_get(mln_p_timeout tm) {
    if (tm->block < 0.0 && tm->total < 0.0) {
        ...
        double t = tm->total - mln_timeout_gettime() + tm->start;
        return MAX(t, 0.0);
...
double mln_timeout_getretry(mln_p_timeout tm) {
    if (tm->block < 0.0 && tm->total < 0.0) {
        ...
        double t = tm->block - mln_timeout_gettime() + tm->start;
        return MAX(t, 0.0);
    } else {
        double t = tm->total - mln_timeout_gettime() + tm->start;
        return MIN(tm->block, MAX(t, 0.0));
...
static int timeout_lua_gettime(lua_State *L)
{
    lua_pushnumber(L, mln_timeout_gettime());
    return 1;
}
