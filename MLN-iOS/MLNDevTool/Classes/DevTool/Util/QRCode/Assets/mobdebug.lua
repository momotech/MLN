    ...
    elseif event == "line" then
        ...
        if models ~= nil and type(models) == "table" then
            ...
            if string.find(file, "%.") then
                _, _, filename = string.find(file, "([/*_*%a+_*/*_*%a+_*]+)") --文件可能含有包名(eg: package/file)
            end
...
local function mln_global_values_valid(name)
    ...
    if mobdebug._global_value_black_list[name] then
        return false
    end
    return true
end
    ...
    while true do
        ...
        elseif command == "STACK" then
            ...
            if ev and ev ~= events.STACK then
                ...
                server:send(vars)
            elseif (is_first_run and not has_breakpoint(sfile, sline)) then
                --即不是step_into也没有设置断点的情况下，只有首次run文件会出现该情况
