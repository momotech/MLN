...
@implementation LNUsbClientImpl
...
- (instancetype)initWithTransporter:(id<LNTransporterProtocol>)transporter listener:(id<LNClientListener>)listener
{
    ...
    if (self) {
        ...
        _writer = [LNWriterFactory getUSBWriter];
...
- (void)writeError:(id)data entryFilePath:(NSString *)entryFilePath {
    [self.writer writeError:data entryFilePath:(NSString *)entryFilePath];
}
...
- (void)onMessage:(void (^)(id))callback {
    [self.reader onMessage:^(id message) {
        if ([message isKindOfClass:[pbdevicecommand class]]) {
            // 更新设备信息
            pbdevicecommand *cmd = (pbdevicecommand *)message;
