...
#ifndef MLN_GCDAsyncSocketLoggingEnabled
#define MLN_GCDAsyncSocketLoggingEnabled 0
#endif

#if MLN_GCDAsyncSocketLoggingEnabled
...
#define LogAsync   YES
#define LogContext MLN_GCDAsyncSocketLoggingContext
...
#ifndef MLN_GCDAsyncSocketLogLevel
#define MLN_GCDAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = MLN_GCDAsyncSocketLogLevel;
...
NSString *const MLN_GCDAsyncSocketException = @"MLN_GCDAsyncSocketException";
NSString *const MLN_GCDAsyncSocketErrorDomain = @"MLN_GCDAsyncSocketErrorDomain";

NSString *const MLN_GCDAsyncSocketQueueName = @"MLN_GCDAsyncSocket";
NSString *const MLN_GCDAsyncSocketThreadName = @"MLN_GCDAsyncSocket-CFStream";

NSString *const MLN_GCDAsyncSocketManuallyEvaluateTrust = @"MLN_GCDAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const MLN_GCDAsyncSocketUseCFStreamForTLS = @"MLN_GCDAsyncSocketUseCFStreamForTLS";
#endif
NSString *const MLN_GCDAsyncSocketSSLPeerID = @"MLN_GCDAsyncSocketSSLPeerID";
NSString *const MLN_GCDAsyncSocketSSLProtocolVersionMin = @"MLN_GCDAsyncSocketSSLProtocolVersionMin";
NSString *const MLN_GCDAsyncSocketSSLProtocolVersionMax = @"MLN_GCDAsyncSocketSSLProtocolVersionMax";
NSString *const MLN_GCDAsyncSocketSSLSessionOptionFalseStart = @"MLN_GCDAsyncSocketSSLSessionOptionFalseStart";
NSString *const MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord = @"MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const MLN_GCDAsyncSocketSSLCipherSuites = @"MLN_GCDAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const MLN_GCDAsyncSocketSSLDiffieHellmanParameters = @"MLN_GCDAsyncSocketSSLDiffieHellmanParameters";
#endif

enum MLN_GCDAsyncSocketFlags
{
...
@interface MLN_MLN_GCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation MLN_MLN_GCDAsyncSocketPreBuffer
...
/**
 * The MLN_GCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface MLN_GCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(MLN_MLN_GCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation MLN_GCDAsyncReadPacket
...
/**
 * The MLN_GCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface MLN_GCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation MLN_GCDAsyncWritePacket
...
/**
 * The MLN_GCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface MLN_GCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation MLN_GCDAsyncSpecialPacket
...
@implementation MLN_GCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<MLN_GCDAsyncSocketDelegate> delegate;
	...
	MLN_GCDAsyncReadPacket *currentRead;
	MLN_GCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	MLN_MLN_GCDAsyncSocketPreBuffer *preBuffer;
	...
	MLN_MLN_GCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id<MLN_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
        NSLog(@"+++ initWithDelegate %@", self);
		delegate = aDelegate;
		delegateQueue = dq;
		...
		{
			socketQueue = dispatch_queue_create([MLN_GCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[MLN_MLN_GCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<MLN_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<MLN_GCDAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError* __autoreleasing *)error
{
  __block BOOL errorOccured = NO;
  
  MLN_GCDAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncSocketOtherError",
                                                           @"MLN_GCDAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<MLN_GCDAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak MLN_GCDAsyncSocket *weakSelf = self;
			
            dispatch_source_set_event_handler(self->accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong MLN_GCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create MLN_GCDAsyncSocket instance for accepted socket
			
			MLN_GCDAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncSocketConnectTimeoutError",
	                                                     @"MLN_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncSocketReadMaxedOutError",
														 @"MLN_GCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncSocketReadTimeoutError",
	                                                     @"MLN_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncSocketWriteTimeoutError",
	                                                     @"MLN_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncSocketClosedError",
	                                                     @"MLN_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MLN_GCDAsyncSocketErrorDomain code:MLN_GCDAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)isAccepted
{
    if (self->accept4Source) {
        return 0 == dispatch_source_testcancel(self->accept4Source);
    }
    return NO;
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the MLN_GCDAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	MLN_GCDAsyncReadPacket *packet = [[MLN_GCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentRead || ![self->currentRead isKindOfClass:[MLN_GCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[MLN_GCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued MLN_GCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued MLN_GCDAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via MLN_GCDAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		MLN_GCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		MLN_GCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MLN_GCDAsyncWritePacket *packet = [[MLN_GCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
        ...
        if (!self->currentWrite || ![self->currentWrite isKindOfClass:[MLN_GCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[MLN_GCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued MLN_GCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued MLN_GCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a MLN_GCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		MLN_GCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	MLN_GCDAsyncSpecialPacket *packet = [[MLN_GCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			MLN_GCDAsyncSpecialPacket *tlsPacket = (MLN_GCDAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:MLN_GCDAsyncSocketUseCFStreamForTLS];
...
{
	MLN_GCDAsyncSocket *asyncSocket = (__bridge MLN_GCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:MLN_GCDAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. MLN_GCDAsyncSocketSSLPeerID
	//  4. MLN_GCDAsyncSocketSSLProtocolVersionMin
	//  5. MLN_GCDAsyncSocketSSLProtocolVersionMax
	//  6. MLN_GCDAsyncSocketSSLSessionOptionFalseStart
	//  7. MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. MLN_GCDAsyncSocketSSLCipherSuites
	//  9. MLN_GCDAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. MLN_GCDAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. MLN_GCDAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. MLN_GCDAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. MLN_GCDAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. MLN_GCDAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:MLN_GCDAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for MLN_GCDAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for MLN_GCDAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use MLN_GCDAsyncSocketSSLProtocolVersionMin & MLN_GCDAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[MLN_MLN_GCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"MLN_GCDAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[MLN_GCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[MLN_GCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("MLN_GCDAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:MLN_GCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(MLN_GCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(MLN_GCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	MLN_GCDAsyncSocket *asyncSocket = (__bridge MLN_GCDAsyncSocket *)pInfo;
