...
@class MLN_GCDAsyncUdpSendPacket;

NSString *const MLN_GCDAsyncUdpSocketException = @"MLN_GCDAsyncUdpSocketException";
NSString *const MLN_GCDAsyncUdpSocketErrorDomain = @"MLN_GCDAsyncUdpSocketErrorDomain";

NSString *const MLN_GCDAsyncUdpSocketQueueName = @"MLN_GCDAsyncUdpSocket";
NSString *const MLN_GCDAsyncUdpSocketThreadName = @"MLN_GCDAsyncUdpSocket-CFStream";

enum MLN_GCDAsyncUdpSocketFlags
{
...
enum MLN_GCDAsyncUdpSocketConfig
{
...
@interface MLN_GCDAsyncUdpSocket ()
{
	...
	MLN_GCDAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	MLN_GCDAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	MLN_GCDAsyncUdpSendPacket *currentSend;
...
/**
 * The MLN_GCDAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface MLN_GCDAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation MLN_GCDAsyncUdpSendPacket
...
@interface MLN_GCDAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation MLN_GCDAsyncUdpSpecialPacket
...
@implementation MLN_GCDAsyncUdpSocket
...
- (id)initWithDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	LogTrace();
...
- (id)initWithDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	LogTrace();
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([MLN_GCDAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (void)setDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)newDelegate synchronously:(BOOL)synchronously
{
...
- (void)setDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id <MLN_GCDAsyncUdpSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)setDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id <MLN_GCDAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MLN_GCDAsyncUdpSocketErrorDomain
	                           code:MLN_GCDAsyncUdpSocketBadConfigError
	                       userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MLN_GCDAsyncUdpSocketErrorDomain
	                           code:MLN_GCDAsyncUdpSocketBadParamError
	                       userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncUdpSocketSendTimeoutError",
	                                                     @"MLN_GCDAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Send operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncUdpSocketErrorDomain
	                           code:MLN_GCDAsyncUdpSocketSendTimeoutError
	                       userInfo:userInfo];
}

- (NSError *)socketClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"MLN_GCDAsyncUdpSocketClosedError",
	                                                     @"MLN_GCDAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:MLN_GCDAsyncUdpSocketErrorDomain code:MLN_GCDAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:MLN_GCDAsyncUdpSocketErrorDomain
	                           code:MLN_GCDAsyncUdpSocketOtherError
	                       userInfo:userInfo];
}
...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError * __autoreleasing *)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
         ...
         *  #535(MLN_GCDAsyncUdpSocket can not send data when data is greater than 9K)
         *
         *
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		MLN_GCDAsyncUdpSpecialPacket *packet = [[MLN_GCDAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[MLN_GCDAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		MLN_GCDAsyncUdpSpecialPacket *connectPacket = (MLN_GCDAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MLN_GCDAsyncUdpSendPacket *packet = [[MLN_GCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	MLN_GCDAsyncUdpSendPacket *packet = [[MLN_GCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [MLN_GCDAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)setSendFilter:(MLN_GCDAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setSendFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setSendFilter:(MLN_GCDAsyncUdpSocketSendFilterBlock)filterBlock
            withQueue:(dispatch_queue_t)filterQueue
       isAsynchronous:(BOOL)isAsynchronous
{
	MLN_GCDAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[MLN_GCDAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			MLN_GCDAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(MLN_GCDAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setReceiveFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setReceiveFilter:(MLN_GCDAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous
{
	MLN_GCDAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
+ (void)listenerThread
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:MLN_GCDAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(MLN_GCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(MLN_GCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		MLN_GCDAsyncUdpSocket *asyncUdpSocket = (__bridge MLN_GCDAsyncUdpSocket *)pInfo;
