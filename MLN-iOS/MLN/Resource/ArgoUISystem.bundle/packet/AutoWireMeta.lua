...
-- __index
--- @param tab table 元表
local function aotuWire__index(tab)
...
-- __newindex
--- @param tab table 元表
local function aotuWire__newindex(tab)
    tab.__newindex = function(t, k, v)
        -- 记录更改位置
        hook_set(tab.__update, k, DATA_UPDATE)
        local newV = AotuWirePack(v)
        hook_set(tab.__o, k, newV)
    end
end

--- @param tab table 元表
local function aotuWire__len(tab)
...
if hook_t == nil then
    ...
    for k, v in pairs(hook_t) do
        table[k] = function(t, ...)
            if hook_get(t, "isAotuWireHook") then
        ...
        for k, v in pairs(o) do
            ...
            if type(v) == "table" then
                if table.getn(v) > 0 then
                    -- 处理未标记改变状态的array，递归查看其下层是否有数据改变
                    if hook_get(meta.__update, k) ~= DATA_UPDATE then
                        local change = changeArray(v);
                        if change == true then
...
--- @param o table 初始化对象
--- 对table进行装包
function AotuWirePack(o)
    -- 已经包装过的，不需要再包装
    if o == nil or type(o) ~= "table" or hook_get(o, "isAotuWireHook") == true then
        return o;
    end
    local newO = { isAotuWireHook = true }
    local meta = getmetatable(o)
    local mt = {
        __mt = meta,
        __o = o, -- 原数据的table
...
local function realAotuWireUnPack(o)
    -- 未包装过的，不需要拆包
    if o == nil or type(o) ~= "table" or getmetatable(o) == nil or hook_get(o, "isAotuWireHook") == nil then
        return o;
    end
    ...
    for k, v in pairs(newO) do
        -- k为索引，v为索引对应的value
        local newV = AotuWireUnPack(v)
        newO[k] = newV -- 重新赋值
    end
    return newO;
end

function AotuWireUnPack(o)
    local table = realAotuWireUnPack(o)
