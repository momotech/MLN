 ...
 */
#ifndef ANIMATOR_SPRINGSPLVER_H
#define ANIMATOR_SPRINGSPLVER_H
...
typedef SSState<Vector4r> SSState4r;
typedef SSDerivative<Vector4r> SSDerivative4r;

  const AMTTimeInterval solverDt = 0.001f;
  const AMTTimeInterval maxSolverDt = 30.0f;
  ...
  class SpringSolver
  {
    ...
    AMTTimeInterval _accumulatedTime;
  ...
  public:
    ...
    void advance(SSState<T> &state, double t, double dt)
    {
      ...
      if (dt > maxSolverDt) {
        ...
        AMTTimeInterval alpha = _accumulatedTime / solverDt;
  ...
  typedef SpringSolver<Vector4r> SpringSolver4r;
