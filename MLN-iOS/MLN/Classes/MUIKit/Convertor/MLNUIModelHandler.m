...
@property (nonatomic, strong) ArgoDataBinding *inner_dataBinding;
@property (nonatomic, strong) NSString *modelKey;
@property (nonatomic, strong) ArgoUIViewLoaderObserver observer;
@end
...
+ (ArgoUIViewLoaderKit *)kit {
    ...
    instance->_innerController = controller; // retain it
    controller.instance = instance;
    return instance;
}

- (ArgoDataBinding *)inner_dataBinding {
    if (!_inner_dataBinding) {
        _inner_dataBinding = [[ArgoDataBinding alloc] init];
    }
...
+ (nullable UIView *)loadViewFromLuaFilePath:(NSString *)filePath withModelKey:(nonnull NSString *)modelKey observer:(nullable ArgoUIViewLoaderObserver)observer {
    ArgoUIViewLoaderKit *kit = [self getInstance];
    kit.modelKey = modelKey;
    kit.observer = observer;
