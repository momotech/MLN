...
- (void)layoutSubviews
{
    ...
    if (_mainView.contentOffset.x == 0  && self.totalItemsCount) {
        int targetIndex = 0;
        if (self.recurrence) {
        ...
        if (targetIndex >= 0 &&  targetIndex < [_mainView numberOfItemsInSection:0]) {
...
{
    MLNCheckTypeAndNilValue(adapter, @"ViewPagerAdapter", MLNViewPagerAdapter)
    _adapter = adapter;
...
{
    self.pageControl.numberOfPages = _showPageControl?self.adapter.cellCounts:0;
...
- (void)beginCycle
{
    ...
    // 在 updateTotalItemCount 中更新_totalItemsCount
    //    _totalItemsCount = self.recurrence ? cellCount * 6 : cellCount;
...
- (void)lua_reloadFinished:(MLNBlock *)block
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        [self beginCycle];
        if (self.reloadFinishedCallback) {
            [self.reloadFinishedCallback callIfCan];
        }
    });
}
...
- (void)scrollWithOldIndex:(NSInteger)fromIndex toIndex:(NSInteger)toIndex progress:(CGFloat)progress
{
    if (_scrollingListerCallback) {
        CGFloat absoluteProgress =  (fromIndex == self.beginIndex) ? progress : (1.0 - progress);
        [_scrollingListerCallback addFloatArgument:absoluteProgress];
        [_scrollingListerCallback addIntegerArgument:self.beginIndex + 1];
        [_scrollingListerCallback addIntegerArgument:(fromIndex==self.beginIndex?toIndex+1:fromIndex+1)];
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger currentPage = ceil(scrollView.contentOffset.x / scrollView.frame.size.width);
