...
static void print_usage (const char *badoption) {
  luai_writestringerror(L, "%s: ", progname);
  if (badoption[1] == 'e' || badoption[1] == 'l')
    luai_writestringerror(L, "'%s' needs argument\n", badoption);
  else
    luai_writestringerror(L, "unrecognized option '%s'\n", badoption);
  luai_writestringerror(L, 
...
int main (int argc, char **argv) {
  printf("in main %d, argv: %s", argc, argv[1]);
  int status, result;
