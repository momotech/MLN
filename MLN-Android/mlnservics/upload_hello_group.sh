...
for ARM in ${arm[*]} ; do
  ...
  echo "----------------------------task: mlnservics clean----------------------------"
  ./../gradlew :mlnservics:clean >/dev/null 2>&1
  echo "----------------------------task: mlnservics uploadArchives ${ARM}----------------------------"
  ./../gradlew :mlnservics:uploadArchives >/dev/null
  uploadResult=$?
