...
static jmethodID setXID;
static int _x(lua_State *L);
static jmethodID getYID;
static jmethodID setYID;
static int _y(lua_State *L);
//</editor-fold>
/**
 * -1: metatable
 */
...
#define JNIMETHODDEFILE(s) Java_com_immomo_mmui_ud_UDPoint_ ## s
/**
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    getXID = (*env)->GetMethodID(env, clz, "getX", "()F");
    setXID = (*env)->GetMethodID(env, clz, "setX", "(F)V");
    getYID = (*env)->GetMethodID(env, clz, "getY", "()F");
    setYID = (*env)->GetMethodID(env, clz, "setY", "(F)V");
}
/**
 * java层需要将此ud注册到虚拟机里
 * @param l 虚拟机
...
static int _x(lua_State *L) {
    ...
    PRE
    if (lua_gettop(L) == 1) {
        jfloat ret = (*env)->CallFloatMethod(env, jobj, getXID);
    ...
    (*env)->CallVoidMethod(env, jobj, setXID, (jfloat)p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".setX")) {
        FREE(env, jobj);
        return lua_error(L);
    }
    FREE(env, jobj);
...
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "setX", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
...
static int _y(lua_State *L) {
    ...
    PRE
    if (lua_gettop(L) == 1) {
        jfloat ret = (*env)->CallFloatMethod(env, jobj, getYID);
    ...
    lua_Number p1 = luaL_checknumber(L, 2);
    (*env)->CallVoidMethod(env, jobj, setYID, (jfloat)p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".setY")) {
        FREE(env, jobj);
        return lua_error(L);
    }
    FREE(env, jobj);
...
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "setY", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
//</editor-fold>
