...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    calculateSizeID = (*env)->GetMethodID(env, clz, "calculateSize", "(F)Lcom/immomo/mmui/ud/UDSize;");
    ...
    showAsImageID = (*env)->GetMethodID(env, clz, "showAsImage", "(Lcom/immomo/mmui/ud/UDSize;)V");
...
/**
 * com.immomo.mmui.ud.UDSize calculateSize(float)
 */
static int _calculateSize(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
...
/**
 * void showAsImage(com.immomo.mmui.ud.UDSize)
 */
static int _showAsImage(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
