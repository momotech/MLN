...
static jmethodID setNineImageID;
static int _setNineImage(lua_State *L);
//</editor-fold>
/**
 * -1: metatable
 */
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    startAnimationImagesID = (*env)->GetMethodID(env, clz, "startAnimationImages", "(Lorg/luaj/vm2/LuaTable;FZ)V");
    stopAnimationImagesID = (*env)->GetMethodID(env, clz, "stopAnimationImages", "()V");
    isAnimatingID = (*env)->GetMethodID(env, clz, "isAnimating", "()Z");
    setNineImageID = (*env)->GetMethodID(env, clz, "setNineImage", "(Ljava/lang/String;)V");
}
/**
 * java层需要将此ud注册到虚拟机里
 * @param l 虚拟机
...
/**
 * void startAnimationImages(org.luaj.vm2.LuaTable,float,boolean)
 */
static int _startAnimationImages(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
...
/**
 * void setNineImage(java.lang.String)
 */
static int _setNineImage(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    PRE
    jstring p1 = lua_isnil(L, 2) ? NULL : newJString(env, lua_tostring(L, 2));
    (*env)->CallVoidMethod(env, jobj, setNineImageID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".setNineImage")) {
        FREE(env, p1);
        FREE(env, jobj);
        return lua_error(L);
    }
...
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "setNineImage", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
//</editor-fold>
