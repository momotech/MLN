...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    watch2ID = (*env)->GetStaticMethodID(env, clz, "watch", "(JLjava/lang/String;IJ)Ljava/lang/String;");
    watchValue0ID = (*env)->GetStaticMethodID(env, clz, "watchValue", "(JLjava/lang/String;J)Ljava/lang/String;");
    watchValue1ID = (*env)->GetStaticMethodID(env, clz, "watchValue", "(JLjava/lang/String;IJ)Ljava/lang/String;");
    watchValue2ID = (*env)->GetStaticMethodID(env, clz, "watchValue", "(JLjava/lang/String;JJ)Ljava/lang/String;");
 ...
 * static java.lang.String watch(long,java.lang.String,int,long)
 */
static int _watch(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    ...
    REMOVE_TOP(L)
    if (lua_gettop(L) == 4) {
        if (lua_type(L, 2) == LUA_TSTRING&&lua_type(L, 3) == LUA_TNUMBER&&lua_isfunction(L, 4)) {
            jlong p1 = (jlong) L;
            jstring p2 = newJString(env, lua_tostring(L, 2));
            lua_Integer p3 = luaL_checkinteger(L, 3);
            luaL_checktype(L, 4, LUA_TFUNCTION);
            jlong p4 = (jlong) copyValueToGNV(L, 4);
            jstring ret = (*env)->CallStaticObjectMethod(env, _globalClass, watch2ID, p1, p2, (jint)p3, p4);
        ...
        dumpParams(L, 2);
        lua_pushfstring(L, LUA_CLASS_NAME ".watch函数3个参数有: (String,int,function) (String,function,function)  ，当前参数不匹配 (%s)", lua_tostring(L, -1));
        setErrorType(L, lua);
         return lua_error(L);
    }
    if (lua_gettop(L) == 3) {
 ...
 * static java.lang.String watchValue(long,java.lang.String,int,long)
...
static int _watchValue(lua_State *L) {
    ...
    if (lua_gettop(L) == 4) {
        if (lua_type(L, 2) == LUA_TSTRING&&lua_type(L, 3) == LUA_TNUMBER&&lua_isfunction(L, 4)) {
            ...
            luaL_checktype(L, 4, LUA_TFUNCTION);
            jlong p4 = (jlong) copyValueToGNV(L, 4);
            jstring ret = (*env)->CallStaticObjectMethod(env, _globalClass, watchValue1ID, p1, p2, (jint)p3, p4);
        ...
        if (lua_type(L, 2) == LUA_TSTRING&&lua_isfunction(L, 3)&&lua_isfunction(L, 4)) {
            ...
            luaL_checktype(L, 4, LUA_TFUNCTION);
            jlong p4 = (jlong) copyValueToGNV(L, 4);
            jstring ret = (*env)->CallStaticObjectMethod(env, _globalClass, watchValue2ID, p1, p2, p3, p4);
            if (catchJavaException(env, L, LUA_CLASS_NAME ".watchValue")) {
                FREE(env, p2);
                return lua_error(L);
            }
        ...
        dumpParams(L, 2);
        lua_pushfstring(L, LUA_CLASS_NAME ".watchValue函数3个参数有: (String,int,function) (String,function,function)  ，当前参数不匹配 (%s)", lua_tostring(L, -1));
        setErrorType(L, lua);
         return lua_error(L);
    }
    if (lua_gettop(L) == 3) {
