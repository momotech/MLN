...
#endif
#define LUA_CLASS_NAME "CollectionViewAdapter"
...
static jmethodID sizeForCellID;
static int _sizeForCell(lua_State *L);
static jmethodID sizeForCellByReuseIdID;
static int _sizeForCellByReuseId(lua_State *L);
static jmethodID setSpanSizeLookUpID;
static int _setSpanSizeLookUp(lua_State *L);
//</editor-fold>
/**
 * -1: metatable
 */
static void fillUDMetatable(lua_State *L, const char *parentMeta) {
    static const luaL_Reg _methohds[] = {
            {"sizeForCell", _sizeForCell},
            {"sizeForCellByReuseId", _sizeForCellByReuseId},
            {"setSpanSizeLookUp", _setSpanSizeLookUp},
            {NULL, NULL}
    };
...
#define JNIMETHODDEFILE(s) Java_com_immomo_mmui_ud_recycler_UDCollectionAdapter_ ## s
/**
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    sizeForCellID = (*env)->GetMethodID(env, clz, "sizeForCell", "(J)V");
    sizeForCellByReuseIdID = (*env)->GetMethodID(env, clz, "sizeForCellByReuseId", "(Ljava/lang/String;J)V");
    setSpanSizeLookUpID = (*env)->GetMethodID(env, clz, "setSpanSizeLookUp", "(J)V");
}
/**
 * java层需要将此ud注册到虚拟机里
 * @param l 虚拟机
...
static int _sizeForCell(lua_State *L) {
    ...
    (*env)->CallVoidMethod(env, jobj, sizeForCellID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".sizeForCell")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "sizeForCell", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void sizeForCellByReuseId(java.lang.String,long)
 */
static int _sizeForCellByReuseId(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    (*env)->CallVoidMethod(env, jobj, sizeForCellByReuseIdID, p1, p2);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".sizeForCellByReuseId")) {
        FREE(env, p1);
        return lua_error(L);
    }
    FREE(env, p1);
...
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "sizeForCellByReuseId", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void setSpanSizeLookUp(long)
 */
static int _setSpanSizeLookUp(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    luaL_checktype(L, 2, LUA_TFUNCTION);
    jlong p1 = (jlong) copyValueToGNV(L, 2);
    (*env)->CallVoidMethod(env, jobj, setSpanSizeLookUpID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".setSpanSizeLookUp")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "setSpanSizeLookUp", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
//</editor-fold>
