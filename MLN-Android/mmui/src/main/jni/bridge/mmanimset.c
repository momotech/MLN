...
#endif
#define LUA_CLASS_NAME "AnimatorSet"
...
static jmethodID togetherID;
static int _together(lua_State *L);
static jmethodID sequentiallyID;
static int _sequentially(lua_State *L);
//</editor-fold>
/**
 * -1: metatable
 */
...
#define JNIMETHODDEFILE(s) Java_com_immomo_mmui_ud_anim_UDAnimatorSet_ ## s
/**
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    togetherID = (*env)->GetMethodID(env, clz, "together", "(Lorg/luaj/vm2/LuaTable;)V");
    sequentiallyID = (*env)->GetMethodID(env, clz, "sequentially", "(Lorg/luaj/vm2/LuaTable;)V");
}
/**
 * java层需要将此ud注册到虚拟机里
 * @param l 虚拟机
...
/**
 * void together(org.luaj.vm2.LuaTable)
 */
static int _together(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    ...
    (*env)->CallVoidMethod(env, jobj, togetherID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".together")) {
        FREE(env, p1);
        return lua_error(L);
    }
    FREE(env, p1);
    ...
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "together", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void sequentially(org.luaj.vm2.LuaTable)
 */
static int _sequentially(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    PRE
    jobject p1 = lua_isnil(L, 2) ? NULL : toJavaValue(env, L, 2);
    (*env)->CallVoidMethod(env, jobj, sequentiallyID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".sequentially")) {
        FREE(env, p1);
        return lua_error(L);
    }
    FREE(env, p1);
...
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "sequentially", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
//</editor-fold>
