...
#endif
#define LUA_CLASS_NAME "SafeAreaAdapter"
...
static jmethodID insetsTopID;
static int _insetsTop(lua_State *L);
static jmethodID insetsBottomID;
static int _insetsBottom(lua_State *L);
static jmethodID insetsLeftID;
static int _insetsLeft(lua_State *L);
static jmethodID insetsRightID;
static int _insetsRight(lua_State *L);
//</editor-fold>
/**
 * -1: metatable
 */
static void fillUDMetatable(lua_State *L, const char *parentMeta) {
    static const luaL_Reg _methohds[] = {
            ...
            {"insetsBottom", _insetsBottom},
            {"insetsLeft", _insetsLeft},
            {"insetsRight", _insetsRight},
            {NULL, NULL}
    };
...
#define JNIMETHODDEFILE(s) Java_com_immomo_mmui_ud_UDSafeAreaRect_ ## s
/**
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    insetsTopID = (*env)->GetMethodID(env, clz, "insetsTop", "(F)V");
    insetsBottomID = (*env)->GetMethodID(env, clz, "insetsBottom", "(F)V");
    insetsLeftID = (*env)->GetMethodID(env, clz, "insetsLeft", "(F)V");
    insetsRightID = (*env)->GetMethodID(env, clz, "insetsRight", "(F)V");
}
/**
 * java层需要将此ud注册到虚拟机里
 * @param l 虚拟机
...
static int _insetsTop(lua_State *L) {
    ...
    (*env)->CallVoidMethod(env, jobj, insetsTopID, (jfloat)p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".insetsTop")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "insetsTop", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void insetsBottom(float)
 */
static int _insetsBottom(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    PRE
    lua_Number p1 = luaL_checknumber(L, 2);
    (*env)->CallVoidMethod(env, jobj, insetsBottomID, (jfloat)p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".insetsBottom")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "insetsBottom", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void insetsLeft(float)
 */
static int _insetsLeft(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    PRE
    lua_Number p1 = luaL_checknumber(L, 2);
    (*env)->CallVoidMethod(env, jobj, insetsLeftID, (jfloat)p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".insetsLeft")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "insetsLeft", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void insetsRight(float)
 */
static int _insetsRight(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    PRE
    lua_Number p1 = luaL_checknumber(L, 2);
    (*env)->CallVoidMethod(env, jobj, insetsRightID, (jfloat)p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".insetsRight")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "insetsRight", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
//</editor-fold>
