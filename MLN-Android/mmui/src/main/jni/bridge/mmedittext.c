...
static jmethodID getReturnModeID;
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    getReturnModeID = (*env)->GetMethodID(env, clz, "getReturnMode", "()I");
...
static int _returnMode(lua_State *L) {
    ...
    PRE
    if (lua_gettop(L) == 1) {
        jint ret = (*env)->CallIntMethod(env, jobj, getReturnModeID);
        if (catchJavaException(env, L, LUA_CLASS_NAME ".getReturnMode")) {
            FREE(env, jobj);
            return lua_error(L);
        }
        FREE(env, jobj);
...
#ifdef STATISTIC_PERFORMANCE
        gettimeofday(&end, NULL);
        userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "getReturnMode", _get_milli_second(&end) - _get_milli_second(&start));
#endif
        return 1;
    }
    lua_Integer p1 = luaL_checkinteger(L, 2);
