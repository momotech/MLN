...
#endif
#define LUA_CLASS_NAME "__BaseNeedHeightAdapter"
#define META_NAME METATABLE_PREFIX "" LUA_CLASS_NAME

static jclass _globalClass;
//<editor-fold desc="method definition">
static jmethodID heightForCellID;
static int _heightForCell(lua_State *L);
static jmethodID heightForHeaderID;
static int _heightForHeader(lua_State *L);
static jmethodID heightForCellByReuseIdID;
static int _heightForCellByReuseId(lua_State *L);
//</editor-fold>
/**
 * -1: metatable
 */
static void fillUDMetatable(lua_State *L, const char *parentMeta) {
    static const luaL_Reg _methohds[] = {
            {"heightForCell", _heightForCell},
            {"heightForHeader", _heightForHeader},
            {"heightForCellByReuseId", _heightForCellByReuseId},
            {NULL, NULL}
    };
...
#define JNIMETHODDEFILE(s) Java_com_immomo_mmui_ud_recycler_UDBaseNeedHeightAdapter_ ## s
/**
...
JNIEXPORT void JNICALL JNIMETHODDEFILE(_1init)
    ...
    heightForCellID = (*env)->GetMethodID(env, clz, "heightForCell", "(J)V");
    heightForHeaderID = (*env)->GetMethodID(env, clz, "heightForHeader", "(J)V");
    heightForCellByReuseIdID = (*env)->GetMethodID(env, clz, "heightForCellByReuseId", "(Ljava/lang/String;J)V");
}
/**
 * java层需要将此ud注册到虚拟机里
 * @param l 虚拟机
...
static int _heightForCell(lua_State *L) {
    ...
    (*env)->CallVoidMethod(env, jobj, heightForCellID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".heightForCell")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "heightForCell", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void heightForHeader(long)
 */
static int _heightForHeader(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    luaL_checktype(L, 2, LUA_TFUNCTION);
    jlong p1 = (jlong) copyValueToGNV(L, 2);
    (*env)->CallVoidMethod(env, jobj, heightForHeaderID, p1);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".heightForHeader")) {
        return lua_error(L);
    }
    lua_settop(L, 1);
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "heightForHeader", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
/**
 * void heightForCellByReuseId(java.lang.String,long)
 */
static int _heightForCellByReuseId(lua_State *L) {
#ifdef STATISTIC_PERFORMANCE
    struct timeval start = {0};
    ...
    (*env)->CallVoidMethod(env, jobj, heightForCellByReuseIdID, p1, p2);
    if (catchJavaException(env, L, LUA_CLASS_NAME ".heightForCellByReuseId")) {
        FREE(env, p1);
        return lua_error(L);
    }
    FREE(env, p1);
...
#ifdef STATISTIC_PERFORMANCE
    gettimeofday(&end, NULL);
    userdataMethodCall(ud->name + strlen(METATABLE_PREFIX), "heightForCellByReuseId", _get_milli_second(&end) - _get_milli_second(&start));
#endif
    return 1;
}
//</editor-fold>
