...
public class DefaultSafeAreaManager implements SafeAreaConstants {
    ...
    public void safeArea(@SafeArea int area, UDLuaView window) {
        ...
        window.getFlexNode().setPadding(YogaEdge.LEFT, areas[0]);
        window.getFlexNode().setPadding(YogaEdge.TOP, areas[1]);
        window.getFlexNode().setPadding(YogaEdge.RIGHT, areas[2]);
        window.getFlexNode().setPadding(YogaEdge.BOTTOM, areas[3]);
        window.getView().requestLayout();
    }
