...
import com.immomo.mmui.ui.IPager;
import com.immomo.mmui.ui.IRefreshRecyclerView;
...
public class UDRecyclerView<T extends ViewGroup & IRefreshRecyclerView & OnLoadListener & ILView,
    ...
    public UDPoint getContentOffset() {
        return new UDPoint(globals,
                DimenUtil.pxToDpi(view.getScrollOffsetX()),
                DimenUtil.pxToDpi(view.getScrollOffsetY()));
    }

    @LuaApiUsed
    public void setContentOffset(UDPoint contentOffset) {
        view.scrollTo(contentOffset.getXPx(), contentOffset.getYPx());
    ...
    public float[] getPagerContentOffset() {
        if (view instanceof IPager) {
            return new float[] {
                    DimenUtil.pxToDpi(((IPager) view).getPagerContentOffsetX()),
                    DimenUtil.pxToDpi(((IPager) view).getPagerContentOffsetY())
            };
        }
        return null;
    }
    ...
    public void setPagerContentOffset(float x, float y) {
        if (view instanceof IPager) {
            ((IPager) view).setPagerContentOffset(
                    DimenUtil.dpiToPx(x),
                    DimenUtil.dpiToPx(y));
        }
    }
    ...
    public void setOffsetWithAnim(UDPoint point) {
        getView().smoothScrollTo(point.getXPx(), point.getYPx());
        point.destroy();
    }
