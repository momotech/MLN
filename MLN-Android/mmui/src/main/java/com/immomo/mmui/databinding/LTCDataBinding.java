...
public class LTCDataBinding {
     ...
     * @return
     */
    @CGenerate(params = "G00F")
    @LuaApiUsed
    static String watch(final long L, String key,final int watchContext, final long fun) {
        if (DataBinding.isLog) {
            Log.d(DataBinding.TAG, "watch---" + key + "---" + fun);
        }
        final Globals globals = Globals.getGlobalsByLState(L);
        return DataBinding.watch(globals, key, new ArgoContextFilter(watchContext), new Callback(globals, fun, key));
    }
    ...
    @CGenerate(params = "G00F")
    @LuaApiUsed
    static String watchValue(final long L, String key, int watchContext, final long callBackFun) {
        if (DataBinding.isLog) {
            Log.d(DataBinding.TAG, "watchValue---" + key + "---");
        }
        final Globals globals = Globals.getGlobalsByLState(L);
        return DataBinding.watchValue(globals, key, new ArgoContextFilter(watchContext), new Callback(globals, callBackFun, key));
    }
    ...
    private static class Callback implements IPropertyCallback {
        ...
        public void callBack(Object old, Object news) {
            ...
            switch (_paramType(news)) {
                case 1:
                    callback.fastInvoke((Boolean) news);
                    break;
                case 2:
                    callback.fastInvoke(((Number) news).doubleValue());
                    break;
                case 3:
                    callback.fastInvoke((Character) news);
                    break;
                case 4:
                    callback.fastInvoke((String) news);
                    break;
                case 5:
                    callback.fastInvoke(BindingConvertUtils.toLuaValue(globals, (ObservableMap) news));
                    break;
                case 6:
                    callback.fastInvoke(BindingConvertUtils.toLuaValue(globals, (ObservableList) news));
                    break;
                case 7:
                    callback.fastInvoke(BindingConvertUtils.toLuaValue(globals, (ObservableField) news));
                    break;
                default:
                    callback.fastInvoke(BindingConvertUtils.toLuaValue(globals, news));
                    break;
            }
        }
         ...
         * 7: field
         */
        private static int _paramType(@NonNull Object a) {
            ...
            if (a instanceof ObservableList)
                return 6;
            if (a instanceof ObservableField)
                return 7;
            return -1;
        }
    ...
    public static class FilterCallBack implements IWatchKeyFilter {
        ...
        public boolean call(int argoWatchContext, String key, int invokeNum) {
            ...
            return callback.fastInvoke_B(argoWatchContext, invokeNum);
        }
    }
}