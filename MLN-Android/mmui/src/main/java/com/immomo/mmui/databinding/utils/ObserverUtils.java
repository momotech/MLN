        ...
        for (ObserverWrap observerWrap : observerWraps) {
            ...
            if ((observerWrap.getSourceTag() + Constants.SPOT).startsWith(changeProperty + Constants.SPOT)) {
                if (observerWrap.getSourceTag().length() == changeProperty.length()) {
                    ...
                    if (observerWrap.isFilter(argoWatchContext,changeProperty)) {
//                        PropertyCallBackHandler.getInstance().addCallBack(WatchCallBack.obtain(observerWrap, older, newer));
                        observerWrap.invokeAdd();
                        observerWrap.getPropertyListener().callBack(older,newer);
                        observerWrap.invokeSub();
                    }
                    ...
                    if (!isItemChange) {
                        ...
                        if (observerWrap.isFilter(argoWatchContext,changeProperty)) {
                            ...
                            observerWrap.invokeSub();
//                            PropertyCallBackHandler.getInstance().addCallBack(WatchCallBack.obtain(observerWrap, olderValue, newerValue));
                        }
                    } else {
                        if (observerWrap.isFilter(argoWatchContext,changeProperty)) {
                            observerWrap.invokeAdd();
                            ...
                            observerWrap.invokeSub();
//                            PropertyCallBackHandler.getInstance().addCallBack(WatchCallBack.obtain(observerWrap, oldClass, newClass));
                        }
                    }
