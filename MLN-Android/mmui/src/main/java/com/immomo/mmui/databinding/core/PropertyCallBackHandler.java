...
public class PropertyCallBackHandler implements Choreographer.FrameCallback {
    ...
    private void doCallBackFrame() {
        ...
        for (WatchCallBack watchCallBack : callBacks) {
            watchCallBack.getObserverWrap().invokeAdd();
            watchCallBack.getObserverWrap().getPropertyListener().callBack(watchCallBack.getOlder(), watchCallBack.getNewer());
            watchCallBack.getObserverWrap().invokeSub();
        }
