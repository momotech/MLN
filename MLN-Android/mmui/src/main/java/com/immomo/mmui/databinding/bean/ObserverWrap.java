...
public class ObserverWrap {
     ...
     * @return
     */
    public boolean isFilter(@WatchContext int argoWatchContext, String key) {
        if (watchKeyFilters == null || watchKeyFilters.size() == 0) {
            return true;
        }

        for (IWatchKeyFilter watchFilter : watchKeyFilters) {
            if (watchFilter != null && !watchFilter.call(argoWatchContext, key,invokeNum)) {
                return false;
            }
        }
        return true;
