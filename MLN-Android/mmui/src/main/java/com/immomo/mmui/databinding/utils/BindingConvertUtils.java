...
public class BindingConvertUtils {
    ...
    public static LuaValue toLuaValue(@NonNull Globals globals, Object value) {
        ...
        if (value instanceof ObservableMap) {
            return toLuaValue(globals, (ObservableMap) value);
        }

        if (value instanceof ObservableList) {
            return toLuaValue(globals, (ObservableList) value);
        }

        if (value instanceof ObservableField) {
            return toLuaValue(globals, (ObservableField) value);
        }

        if (value instanceof IUserdataHolder) {
            return ((IUserdataHolder) value).getUserdata();
        }

        Translator t = Translator.fromGlobals(globals);
        if (t != null)
            return t.translateJavaToLua(globals, value);
        return LuaValue.Nil();
    }

    public static LuaValue toLuaValue(Globals globals, ObservableList list) {
        LuaTable cache = list.getFieldCache(globals);
        if (cache != null) {
        ...
        return toTable(globals, list);
    }

    public static LuaValue toLuaValue(Globals globals, ObservableMap map) {
        LuaTable cache = map.getFieldCache(globals);
        if (cache != null) {
            if (DataBinding.isLog) {
                Log.d(DataBinding.TAG, "from cache");
            }
        ...
        return toTable(globals, map);
    }

    public static LuaValue toLuaValue(Globals globals, ObservableField field) {
        LuaTable cache = field.getFieldCache(globals);
        if (cache != null) {
            if (DataBinding.isLog) {
                Log.d(DataBinding.TAG, "from cache");
            }
        ...
        return toTable(globals, (ObservableMap) field.getFields());
    }
