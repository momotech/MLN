...
import com.immomo.mls.ILuaLifeCycle;
...
public class MMUIInstance implements ScriptLoader.Callback, Callback, PrinterContainer,
    ...
    private boolean imediatelyReload = true;
    /**
     * 加载失败显示的视图
     *
    ...
    public MMUIInstance(@NonNull Context context, boolean isChild) {
        this(context, isChild, false, MLSEngine.DEBUG);
    }

    public MMUIInstance(@NonNull Context context, boolean isChild, boolean isHotReloadPage, boolean showDebugButton) {
    ...
    private void createLuaViewManager() {
        if (luaViewManager == null) {
            luaViewManager = new MMUILuaViewManager(mContext, MMUIEngine.singleTranslator);
    ...
    private void reloadByHotReload(String url, int type, HashMap<String, String> params) {
        ...
        if (!imediatelyReload) {
            if (hotReloadLuaViewManager == null) {
                hotReloadLuaViewManager = new MMUILuaViewManager(mContext, MMUIEngine.singleTranslator);
    ...
    private void onLuaScriptExecutedSuccess() {
        ...
        if (MLNCore.DEBUG) {
            LogUtil.d(">>>all vm use " + MemoryMonitor.getMemorySizeString(Globals.getAllLVMMemUse())
                    + " memory, " + globals
    ...
    public void onScriptLoadSuccess(final ScriptBundle scriptBundle) {
        ...
        Runnable task = new Runnable() {
            ...
            public void run() {
                ...
                ScriptLoader.loadScriptBundle(scriptBundle, g, MMUIInstance.this, getUniqueTag());
                if (MLSEngine.DEBUG && !isChild) {
