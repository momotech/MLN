...
public class AutoFillConvertUtils {
    ...
    Object toNativeValue(@Nullable LuaValue value) {
        ...
        switch (value.type()) {
            ...
            case LuaValue.LUA_TLIGHTUSERDATA:
                ...
                return MMUIEngine.singleTranslator.translateLuaToJava(ud, jud != null ? jud.getClass() : null);
        }
        return value;
    }
    ...
    /**  --------------------------------------------------------------------------------------------------------------------------
     * -------------------------  以下方法为针对lua的map中数据更改标志位对viewModel进行赋值的新增方法   ----------------------------------
     * ---------------------------------------------------------------------------------------------------------------------------- */

    /**
     * 模拟lua元表，为每一个table保存一个关联的原表对应类
     */
    private static @Nullable
    Object toNativeValue(@Nullable LuaValue value, Map<Object, Object> update) {
        if (value == null || value.isNil())
            return null;
        if (value instanceof UDView)
            return value;
        switch (value.type()) {
            ...
            case LuaValue.LUA_TTABLE:
                ...
                if (luaTable.isEmpty()) {
                    ...
                    return luaTable.getn() > 0 ? toList(luaTable, update) : toMap(luaTable, update);
                }
            case LuaValue.LUA_TUSERDATA:
            case LuaValue.LUA_TLIGHTUSERDATA:
                LuaUserdata ud = value.toUserdata();
    ...
    List<Object> getUpdateList(LuaTable table) {
        ArrayList ret = new ArrayList();
        if (table != null) {
            LuaTable metatable = table.getMetatable();
            LuaTable luaTable = (LuaTable) metatable.get("__update");
            List<Object> list = toList(luaTable);
            ret.addAll(list);
        }
        return ret;
    }
    ...
    /**
     * 生成table数据，并保存对应的元表（数据修改信息类）
     * @param table 数据表
     * @param update 所有元表的存储类
     * @return map
     */
    private static @NonNull
    List<Object> toList(LuaTable table, Map<Object, Object> update) {
        ...
        if (table != null) {
            ...
            if (iterator != null) {
                while (iterator.hasNext()) {
                    LuaTable.KV kv = iterator.next();
                    ret.add(toNativeValue(kv.value, update));
                }
                iterator.dispose();
            }
        }
        // 模拟lua元表，为每一个table保存一个关联的原表对应类
        List<Object> updateList = getUpdateList(table);
        update.put(ret, updateList);
        return ret;
    }
     ...
     */
    private static @NonNull
    Map<String, Object> getUpdateMap(LuaTable table) {
        HashMap<String, Object> ret = new HashMap<>();
        if (table != null) {
            LuaTable metatable = table.getMetatable();
            LuaTable luaTable = (LuaTable) metatable.get("__update");
            Map<String, Object> map = toMap(luaTable);
            ret.putAll(map);
        }
        return ret;
    }
     ...
     */
    private static @NonNull
    Map<String, Object> toMap(LuaTable table, Map<Object, Object> update) {
        HashMap<String, Object> ret = new HashMap<>();
        if (table != null) {
            DisposableIterator<LuaTable.KV> iterator = table.iterator();
            if (iterator != null) {
                while (iterator.hasNext()) {
                    LuaTable.KV kv = iterator.next();
                    String key = kv.key.toJavaString();
                    ret.put(key, toNativeValue(kv.value, update));
                }
                iterator.dispose();
            }
        }
        // 模拟lua元表，为每一个table保存一个关联的原表对应类
        Map<String, Object> updateMap = getUpdateMap(table);
        update.put(ret, updateMap);
        return ret;
    }

    /**
     * 生成带有存对应的元表（数据修改信息类）的table数据的总入口
     * @param table 数据表
     */
    public static @NonNull
    AutoFillData toAutoData(LuaTable table) {
        AutoFillData data = new AutoFillData();
        Map<Object, Object> update = new HashMap<>();
        data.setUpdate(update);
        Map<String, Object> ret = toMap(table, update);
        data.setData(ret);
        return data;
    }

    /**  --------------------------------------------------------------------------------------------------------------------------
     * -------------------------  以上方法为针对lua的map中数据更改标志位对viewModel进行赋值的新增方法   ----------------------------------
     * ---------------------------------------------------------------------------------------------------------------------------- */
