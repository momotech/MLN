...
public class NodeLayout extends ViewGroup implements IFlexLayout {
    ...
    // 防止虚拟布局的transfer，影响逻辑位置，用index获取node的view
    // 判断view 如果是虚拟view，把transfer后的子View也移除
    public View findRealView(int index) {
        if (mNode.getChildCount() <= index) {
            return null;
        }
        FlexNode childNode = mNode.getChildAt(index);
        Object view = childNode.getData();

        if (view != null && VirtualViewUtil.isNotVirtualView(view)) {//非虚拟 或 普通view
            return (View) childNode.getData();
        }
        return null;
    }
    ...
    public void removeViewAt(int index) {
        ...
        removeViewFromYogaTree(findRealView(index), false);
    ...
    public void removeViews(int start, int count) {
        for (int i = start + count - 1; i >= start; i--) {
            removeViewFromYogaTree(findRealView(i), false);
        }
    ...
    public void removeViewsInLayout(int start, int count) {
        for (int i = start + count - 1; i >= start; i--) {
            removeViewFromYogaTree(findRealView(i), true);
        }
    ...
    protected void removeViewFromYogaTree(View view, boolean inLayout) {
        ...
        boolean isVirtualOwner = owner != null && owner != mNode;//owner为虚拟
        ...
        if (owner != null) {
            ...
            // 判断view 如果是虚拟view，把transfer后的子View也移除(有owner && 虚拟，必然转移了)
            if (view instanceof LuaNodeLayout && ((LuaNodeLayout) view).isVirtual()) {
    ...
    private void applyLayoutRecursive(FlexNode node, float xOffset, float yOffset) {
        ...
        if (view != null && VirtualViewUtil.isNotVirtualView(view) && view != this) {
        ...
        for (int i = 0; i < childrenCount; i++) {
            ...
            } else if (view instanceof VirtualLayout && !((VirtualLayout) view).isVirtual()) {
                continue;
            } else {
