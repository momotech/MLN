...
public class UDSafeAreaRect extends LuaUserdata {
    ...
    public LuaValue[] insetsTop(LuaValue[] v) {
        safeArea.top = v.length > 0 ? DimenUtil.dpiToPx(v[0].toInt()) : 0;
        return null;
    }

    @LuaApiUsed
    public LuaValue[] insetsBottom(LuaValue[] v) {
        safeArea.bottom = v.length > 0 ? DimenUtil.dpiToPx(v[0].toInt()) : 0;
        return null;
    }

    @LuaApiUsed
    public LuaValue[] insetsLeft(LuaValue[] v) {
        safeArea.left = v.length > 0 ? DimenUtil.dpiToPx(v[0].toInt()) : 0;
        return null;
    }

    @LuaApiUsed
    public LuaValue[] insetsRight(LuaValue[] v) {
        safeArea.right = v.length > 0 ? DimenUtil.dpiToPx(v[0].toInt()) : 0;
        return null;
    }
