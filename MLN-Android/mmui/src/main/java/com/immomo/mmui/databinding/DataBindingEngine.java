...
import com.immomo.mmui.databinding.interfaces.ILuaCallback;
...
public class DataBindingEngine {
    ...
    private Map<Globals, ILuaCallback> callbackMap = new HashMap<>();
...
//        iGetSet = new GetSetReflexAdapter();
    ...
    /**
     * 给lua虚拟机绑定一个统一的lua回调接口，业务方进行分发
     *
     * @param target globals
     * @param callback 回调
     */
    public void registerCallback(Globals target, ILuaCallback callback) {
        callbackMap.put(target, callback);
    }
    ...
    public void update(Globals globals, String tag, Object propertyValue) {
        ...
        ILuaCallback iLuaCallback = callbackMap.get(globals);
        if (iLuaCallback != null) {
            iLuaCallback.callback(tag, propertyValue);
        }
    ...
    public String getLastTag(DataSource dataSource, String tag) {
        ...
        if (!TextUtils.isEmpty(listTag)) { // tag中 包含list
            ...
            if (!DataBindUtils.isDoubleList(observableList) && restTags.length > 1 && DataBindUtils.isNumber(restTags[1])) { // 如果是一维数组，且第二个是数字，去掉第一位
                String newTag = listTag + Constants.SPOT + restTag.substring(restTag.indexOf(Constants.SPOT) + 1);
                return getLastTag(dataSource,newTag);
            } else {
                return tag;
            }
        } else {
