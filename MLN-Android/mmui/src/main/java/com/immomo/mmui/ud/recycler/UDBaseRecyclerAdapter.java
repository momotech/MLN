...
public abstract class UDBaseRecyclerAdapter<L extends UDBaseRecyclerLayout> extends JavaUserdata implements OnLoadListener {
        ...
        if (headerValidDelegate != null) {
            ...
            if (valid) {
                if (mAdapter != null && mAdapter.getHeaderCount() == 0) {
                    mAdapter.setHeaderView(new FrameLayout(getContext()));
                }
                return;
            }
    ...
    protected String getReuseId(int position) {
        ...
        if (reuseIdDelegate != null && !reuseIdDelegate.isNil()) {
            ...
            if (MLSEngine.DEBUG && TextUtils.isEmpty(result)) {//统一报错，reuseid不能为""
                IllegalArgumentException e = new IllegalArgumentException("reuseId不能为空字符串:" + reuseIdDelegate.getSource());
                if (!Environment.hook(e, getGlobals())) {
                    throw e;
                }
            }
    ...
    private boolean canDoClick() {
        MMUILuaViewManager m = (MMUILuaViewManager) getGlobals().getJavaUserdata();
        MMUIInstance instance = m != null ? m.instance : null;
        if (instance != null) {
