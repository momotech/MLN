...
local __OperationType = {
    WT_normal = 1, -- __watch1 -- 普通watch 监听最后一个key的值变化 a.b.c 只有c变化时才走回调
...
local function bindMeta__index(t, k)
    ...
    elseif k == __ci or k == __cii then
        ...
        return BindMeta(bindMeta_path(mt.__kvoname),
                {row={__get=mt.__ck}, section={__get=section}}, temp_v, mt.__ck, mt.__pk)
    end
...
---
--- 获取foreach的缓存
--- @param t table weak K  {view, {{view = v} }}
function BindMetaForeachGetT(t, view, index)
    local ret = t[view]
    if ret == nil then
        ret = {}
        t[view] = ret
