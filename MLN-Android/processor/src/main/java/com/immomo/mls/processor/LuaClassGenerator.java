...
class LuaClassGenerator {
    ...
    public static final ClassName LuaApiUsed = ClassName.get(PACKAGE_NAME + ".utils", "LuaApiUsed");
    ...
    private void addPropertyImpl(MethodSpec.Builder mb, VariableElement e, boolean sb) {
        ...
        if (isLuaValue(type)) {
            ...
            mb.addCode("if (p == null || p.length == 0) \n\treturn LuaValue.varargsOf");
            if (sb) {
                mb.addStatement("($T.fromGlobals(globals).translateJavaToLua(globals, $N))", UserdataTranslator, call)
                        .addStatement("$N = ($T)$T.fromGlobals(globals).translateLuaToJava(p[0], $T.class)", call, type, UserdataTranslator, cn);
            } else {
                mb.addStatement("($T.fromGlobals(getGlobals()).translateJavaToLua(globals, $N))", UserdataTranslator, call)
                        .addStatement("$N = ($T)$T.fromGlobals(getGlobals()).translateLuaToJava(p[0], $T.class)", call, type, UserdataTranslator, cn);
            }
        }
    ...
    private void addPropertyMethodImpl(MethodSpec.Builder mb, ExecutableElement getter, ExecutableElement setter, boolean sb) {
        ...
        if (isLuaValue(type)) {
            ...
            if (sb) {
                mb.addStatement("$T globals = $T.getGlobalsByLState(L)", Globals, Globals);
                mb.addStatement("($T.fromGlobals(globals).translateJavaToLua(globals, $N))", UserdataTranslator, gcall)
                        .addStatement(String.format(scall, "($T)$T.fromGlobals(globals).translateLuaToJava(p[0], $T.class)"), type, UserdataTranslator, cn);
            } else {
                mb.addStatement("($T.fromGlobals(getGlobals()).translateJavaToLua(globals, $N))", UserdataTranslator, gcall)
                        .addStatement(String.format(scall, "($T)$T.fromGlobals(getGlobals()).translateLuaToJava(p[0], $T.class)"), type, UserdataTranslator, cn);
            }
        }
        mb.addStatement("return null");
    }
     ...
     * 参数中是否需要globals
     */
    private boolean needGlobals(List<? extends VariableElement> types) {
        for (VariableElement e : types) {
            TypeName vt = TypeName.get(e.asType());
            if (!(vt.isPrimitive()
                    || vt.isBoxedPrimitive()
                    || isString(vt)
                    || isLuaValue(vt)
                    || vt instanceof ArrayTypeName))
                return true;
    ...
    private void addMethodImpl(MethodSpec.Builder mb, ExecutableElement e, boolean sb) {
        ...
        for (int i = 0; i < plen; i++) {
            ...
            if (Globals.equals(vt)) {
                ...
                if (sb) {
                    mb.addStatement("((p.length > $L && !p[$L].isNil()) ? ($T)$T.fromGlobals(globals).translateLuaToJava(p[$L], $T.class) : null)", index, index, vt, UserdataTranslator, index, cn);
                } else {
                    mb.addStatement("((p.length > $L && !p[$L].isNil()) ? ($T)$T.fromGlobals(getGlobals()).translateLuaToJava(p[$L], $T.class) : null)", index, index, vt, UserdataTranslator, index, cn);
                }
            }
        }
        /**
    ...
    private static String translateJavaToLua(String s) {//UserdataTranslator
        return "$T.fromGlobals(globals).translateJavaToLua(globals, " + s + ")";
    }
    ...
    static class Builder {
        ...
        Builder setParent(LuaClassGenerator parent) {
            this.parent = parent;
            return this;
        }
        ...
        LuaClassGenerator build() {
            LuaClassGenerator r = new LuaClassGenerator();
            r.type = t;
            r.typeElement = type;
