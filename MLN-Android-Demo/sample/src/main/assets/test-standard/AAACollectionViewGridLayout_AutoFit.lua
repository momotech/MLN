...
--- Created by Feng.
--- DateTime: 2019/3/6 上午11:10
---
--local r = require('print_table')
adapter = CollectionViewAutoFitAdapter()
adapter:pressedColor(Color(255,255,255,1))
print('adapter pressed color:', adapter:pressedColor())
...
ta:call()
tb:call()
local size = ta:getSize()
local sizeb = ta:getSize()
...
collectionView:setContentInset(10,20,40,50)

collectionView:getContentInset(function(top,right,bottom,left)
    print('getContentInset top = '..tostring(top)..'  right = '..tostring(right)..' bottom='..tostring(bottom)..' left='..tostring(left))
end)
...
adapter:initCell(function(cell)
    ...
    contentView:addShadow(Color(255,0,0,1), Size(0,0),30,1)
end)
...
insertsLabel:onClick(function()
    ...
    rowCount = rowCount + 4
    collectionView:insertRowsAtSection(1,1,4,true)
end)
...
deletesLabel:onClick(function()
    ...
    table.remove(Data.sections[1].items, 1)
    table.remove(Data.sections[1].items, 1)
    collectionView:deleteRowsAtSection(1,1,4,true)
end)
