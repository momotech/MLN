...
adapter:cellDidDisappearByReuseId("1", function(cell, section, row)
    print('cell disappear "1"', cell, section, row)
end)
adapter:cellDidDisappearByReuseId("2", function(cell, section, row)
    print('cell disappear "2"', cell, section, row)
end)

adapter:cellWillAppear(function(cell, section, row)
    print('cell appear', cell, section, row)
end)
adapter:cellWillAppearByReuseId("1", function(cell, section, row)
    print('cell appear "1"', cell, section, row)
end)
adapter:cellWillAppearByReuseId("2", function(cell, section, row)
    print('cell appear "2"', cell, section, row)
end)
...
adapter:initCellByReuseId('1', function(cell)
    ...
    cell.contentView:bgColor(Color(123,123,123,1))
end)
...
adapter:initHeader(function(cell)
    local contentView = cell.contentView
    cell.contentView:bgColor(Color(255,0,123,1))
    --normalCell:onCreate(cell, nil)
    cell.label = Label():width(100):height(90)
    contentView:addView(cell.label)

end)

adapter:fillHeaderData(function(cell, section, row)
    cell.label:text("hahaha ")
end)

adapter:heightForHeader(function(section, row)
    return 120
end)
...
collectionView:setRefreshingCallback(function()
    ...
    TimeManager:setTimeOut(function()
        ...
        --collectionView:setScrollEnable(canScroll)
